...
public class ReferralsController : BaseApiController
{
    ...
    {
        var result = await _referralService.GetSummaryAsync(GetCurrentUserId());
    ...
    {
        var result = await _referralService.GetHistoryAsync(GetCurrentUserId(), Math.Clamp(limit, 1, 100));
        return HandleResult(result);
    }
}
