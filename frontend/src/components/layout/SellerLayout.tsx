...
import { useGetSellerProfileQuery } from '@/features/seller/sellerApi';
import {
...
const sellerNavigationGroups: DashboardNavGroup[] = [
  {
    label: 'Genel',
    items: [
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Ürün Ekle', href: '/seller/products/new', icon: PlusCircle },
      { label: 'Stok Yönetimi', href: '/seller/products', icon: Truck, badge: 'MVP' },
    ],
  },
  {
    label: 'Satışlar',
  ...
  },
  {
    label: 'Finans',
    items: [
      { label: 'Kazancım & Komisyon', href: '/seller/finance', icon: Wallet, badge: 'MVP' },
    ],
  },
  {
  ...
  },
  {
    label: 'Mağaza',
    items: [
      { label: 'Mağaza Profili', href: '/seller/profile', icon: User },
      { label: 'Başvuru Durumu', href: '/seller/profile', icon: Store, badge: 'MVP' },
    ],
  },
];
              ...
              const itemClasses = cn(
                ...
                item.disabled
                  ...
                  : isActive
                    ...
                    : 'text-muted-foreground hover:bg-amber-100/70 hover:text-foreground dark:hover:bg-amber-950/30'
              );
...
export function SellerLayout() {
  ...
  const [collapsed, setCollapsed] = useState(() => localStorage.getItem('sellerSidebarCollapsed') === 'true');

  useEffect(() => {
    localStorage.setItem('sellerSidebarCollapsed', String(collapsed));
  }, [collapsed]);

  const normalizedPath = normalizeDashboardPath(location.pathname, '/seller');
  const breadcrumbs = buildDashboardBreadcrumbs(normalizedPath, '/seller', 'Satıcı Paneli', sellerNavigationGroups);
  const unreadNotificationCount = notificationSummary?.unreadCount ?? 0;
  const storeName = profile?.brandName || `${user?.firstName ?? 'Satıcı'} ${user?.lastName ?? ''}`.trim();
  ...
  return (
    <div className="min-h-screen bg-amber-50/30 dark:bg-amber-950/10 md:flex">
      <aside
        className={cn(
      ...
      >
        <div className={cn('flex items-center border-b px-3 py-3', collapsed ? 'justify-center' : 'justify-between')}>
          {!collapsed ? (
            <div>
              ...
              <h2 className="mt-1 text-lg font-semibold">{storeName || 'Satıcı Paneli'}</h2>
            </div>
          ) : (
        ...
        <ScrollArea className="flex-1">
          <SellerNav collapsed={collapsed} pathname={normalizedPath} />
        </ScrollArea>
            ...
            <div className="flex items-center gap-3">
              <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
                ...
                <SheetContent side="left" className="w-80 p-0">
                  <SheetTitle className="sr-only">Satıcı menüsü</SheetTitle>
                  <SheetDescription className="sr-only">Satıcı panel navigasyonu</SheetDescription>
                  <div className="border-b px-4 py-4">
                    <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Seller</p>
                    <h2 className="mt-1 text-lg font-semibold">{storeName || 'Satıcı Paneli'}</h2>
                  </div>
                  <ScrollArea className="h-[calc(100vh-8rem)]">
                    <SellerNav collapsed={false} onNavigate={() => setMobileMenuOpen(false)} pathname={normalizedPath} />
                  </ScrollArea>
                </SheetContent>
              </Sheet>
              <div>
                <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Seller</p>
                <p className="font-semibold">{storeName || 'Satıcı Paneli'}</p>
              </div>
            </div>
          ...
          <div className="hidden h-16 items-center justify-between gap-6 px-6 md:flex">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Mağazanızı, ürünlerinizi ve operasyonel performansınızı buradan yönetin.
              </p>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" className="flex h-auto items-center gap-3 rounded-2xl px-2 py-1.5">
                    <Avatar className="h-9 w-9 border">
                      <AvatarFallback className="bg-amber-500/10 font-semibold text-amber-700 dark:text-amber-300">
                ...
                <DropdownMenuContent align="end" className="w-64">
                  <DropdownMenuLabel>Satıcı Hesabı</DropdownMenuLabel>
                  <DropdownMenuSeparator />
